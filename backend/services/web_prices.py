...
# Число з роздільником тисяч або без нього. Варіант із роздільником має
# стояти першим, інакше «1 250» прочиталось би як «1», а «2600» — як «260».
_NUMBER = r"\d{1,3}(?:[\s\u00a0 ]\d{3})+(?:[.,]\d+)?|\d+(?:[.,]\d+)?"

_PRICE_RE = re.compile(
    r"(?P<price>" + _NUMBER + r")\s*"
    r"(?:грн|₴|uah)\.?\s*"
    r"(?:/|за\s+|/\s*)?\s*"
...
_RANGE_RE = re.compile(
    r"(?P<a>" + _NUMBER + r")\s*[-–—]\s*(?P<b>" + _NUMBER + r")\s*(?:грн|₴)", re.I)
