...
## Швидкий шлях: усе з браузера

Google дає **Cloud Shell** — термінал прямо в консолі, з готовим `gcloud` і без
потреби щось ставити на свій комп'ютер. Це найкоротший і найменш помилковий
шлях: скрипт сам обере безкоштовну конфігурацію, створить правила фаєрвола і
закріпить адресу.

1. Зареєструйтесь на <https://console.cloud.google.com>, створіть проєкт
   (**Select a project → New project**, назва `budsmet`).
2. Натисніть **`>_`** праворуч угорі — відкриється Cloud Shell.
3. Виконайте в ньому:

```bash
git clone --depth 1 --branch claude/web-app-cost-estimates-l1x9ef \
  https://github.com/KAZUM0RA/Budsmet.git ~/budsmet && \
~/budsmet/deploy/gcp/create-vm.sh
```

Скрипт створить машину `e2-micro` у зоні `us-central1-a`, відкриє порти 80 і
443, закріпить зовнішню адресу і наприкінці покаже IP та наступні дві команди.
Якщо випадково вкажете платний регіон або завеликий диск — він попередить і
перепитає, перш ніж щось створювати.

Далі — [крок 4 (домен)](#крок-4-домен) і [крок 5 (встановлення)](#крок-5-встановлення).

---

## Повільний шлях: через форму в консолі

Те саме, але клацанням у веб-інтерфейсі — якщо Cloud Shell чомусь не
підходить.
...
Якщо підключатиметесь командою `gcloud compute ssh` (у Cloud Shell або зі свого
комп'ютера), ключ створювати не треба — Google зробить це сам при першому
підключенні. Цей крок потрібен лише для входу звичайним `ssh`:
...
```bash
gcloud compute ssh budsmet --zone=us-central1-a     # або: ssh ubuntu@ВАШ_EXTERNAL_IP
```
