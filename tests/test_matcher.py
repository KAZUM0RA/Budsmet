...
# ------------------------------------------------- довгі назви з АВК-5 і хвости

# У відомостях і дефектних актах назва роботи часто має хвіст-уточнення:
# ділянку, поверхню, обсяг в одному місці. Довідник тримає коротку назву.
AVK_TAILS = [
    ("Розбирання кам'яної кладки простих стін із цегли (парапети сходів)",
     "м3", "розбирання кам'яної кладки"),
    ("Розбирання покриттів підлог з керамічних плиток (сходи, ганок, пандус)",
     "м2", "розбирання покриттів підлог"),
    ("Відбивання штукатурки по цеглі та бетону зі стін та стель, площа"
     " відбивання в одному місці до 5 м2 (цоколь сходів)",
     "м2", "відбивання штукатурки"),
    ("Облицювання стін фасадів будівель керамічною плиткою на цементному"
     " розчині по бетонній і цегляній поверхні (цоколь)",
     "м2", "облицювання стін фасадів"),
    ("Улаштування основи під штукатурку з металевої сітки по цегляних та"
     " бетонних поверхнях",
     "м2", "основи під штукатурку з металевої сітки"),
]


@pytest.mark.parametrize("query,unit,expected", AVK_TAILS)
def test_qualifying_tail_does_not_sink_the_match(query, unit, expected):
    """Хвіст-уточнення не має топити розцінку, яка цілком міститься в назві."""
    best = matcher().best(query, unit)
    assert best is not None, f"не підібрано розцінку для «{query}»"
    assert expected in best.name.lower(), f"«{query}» → «{best.name}»"
    assert best.score >= 80.0, f"«{query}» → «{best.name}» лише {best.score}"


def test_query_unknown_to_the_catalog_is_not_matched():
    """Один знайомий іменник серед чужих слів — не підстава для розцінки.

    Інакше «трап» у «Шліфування космічного трапа» дав би повне покриття й
    програма підставила б у кошторис упевнену, але зовсім чужу ціну.
    """
    assert matcher().best("Шліфування космічного трапа", "м2") is None


def test_demolition_never_matches_facing_work():
    """«Облицювання» — монтажна дія, демонтажний запит не має її обирати."""
    best = matcher().best("Демонтаж старої плитки зі стін", "м2")
    assert best is not None
    assert "розбирання" in best.name.lower(), f"→ «{best.name}»"
